...
function theme_mb2nl_course_sections_accordion()
{
	...
	$output .= '<div class="accordion mb2-accordion theme-enrol style-default sizem rounded1" id="accordion-course-' . $COURSE->id . '">';
	...
	foreach ( $sections as $section )
	{
		...
		$output .= '<h5>';
		$output .= '<button class="themereset' . $collapsedcls . '" data-toggle="collapse" data-target="#' . $collid. '" aria-expanded="' . $isexpand . '" aria-controls="' . $collid. '">';
		$output .= $section['name'];
		$output .= '</button>';
		$output .= '</h5>';
		$output .= '</div>';
 ...
 *
 */
function theme_mb2nl_get_content_field_textarea( $content = '', $courseid = 0, $fieldid  )
{
	global $COURSE, $CFG;
	require_once( $CFG->libdir . '/filelib.php' );
...
function theme_mb2nl_mb2fields_filed($name, $courseid = 0)
{
	...
	foreach ( $fields as $k=>$f )
	{
		...
		elseif ( $name === 'mb2video_local' || $name === 'mb2image' )
		{
			return theme_mb2nl_text2url($val);
		}
		else
		{
...
/*
 *
 * Method to get local vide url from video source code
 *
 */
function theme_mb2nl_text2url($video)
{
...
function theme_mb2nl_course_quickview( $courseid )
{
	...
	if ( $settings->skills )
	{
		$skills = theme_mb2nl_skills_list( $settings->skills, false, 3 );
	}
	elseif ( $fieldskills )
	{
		$skills = theme_mb2nl_skills_list( $fieldskills, false, 3 );
	}
...
function theme_mb2nl_block_enrol($video = false)
{
	...
	if ( theme_mb2nl_is_course_price() )
	{
		...
		$output .= '<span class="price-label">' . get_string('price','theme_mb2nl') . '</span>';
	...
	$output .= '<a href="#page-content" class="mb2-pb-btn typeprimary sizelg course-enrolbtn sidebar-btn fwmedium">';
 ...
 *
 */
function theme_mb2nl_skills_list( $text, $columns = true, $limit = 999 )
{
	...
	foreach ( $content as $item )
	{
		...
		$output .= '<i class="ri-check-line"></i>';
		$output .= '</span>';
...
function theme_mb2nl_course_intro( $course = NULL, $settings )
{
	$intro = '';
